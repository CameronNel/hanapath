...
- **App shape:** vanilla static PWA — **no build step, no framework, no `package.json`**. The core is one large file `app.js` (~13.7k lines) loaded as a plain browser script, plus:
  ...
  - `lib/hangul.js`
  - `words_curated_core.js` — 1,996 curated Words rows (`window.HANAPATH_CURATED_WORDS`) — but never trust this cached number; the audit's `Curated words:` line is ground truth
...
- **What remains to finalize the Words section** — the authoritative, ordered checklist is **teaching spec §9**. Summary:
  1. **M2 sense split** — continue the high-frequency polyseme list in spec §9. 85 lemmas are genuinely split as of 2026-07-04 (roadmap Track B1 done; B2–B5 remain); the full lemma list lives in spec §8 (single source of truth — don't maintain a copy here).
