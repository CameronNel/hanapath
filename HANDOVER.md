...
- **App shape:** vanilla static PWA — **no build step, no framework, no `package.json`**. The core is one large file `app.js` (~13.7k lines) loaded as a plain browser script, plus:
  ...
  - `lib/hangul.js`
  - `words_curated_core.js` — ~1,900 curated Words rows (`window.HANAPATH_CURATED_WORDS`)
...
- **What remains to finalize the Words section** — the authoritative, ordered checklist is **teaching spec §9**, and the batch-by-batch execution queue for it is [`docs/WORDS_FINAL_ROADMAP.md`](docs/WORDS_FINAL_ROADMAP.md) (one unchecked box = one PR; start there). Summary as of 2026-07-03:
  1. **M2 sense split** — **44 lemmas** have genuine 2+ sense rows (grown through PRs #57, #60–#62); the remaining vetted candidates are roadmap Track B.
  2. **Curriculum polish** — ✅ done (#59): subtitle/count mismatches fixed, thin lessons folded, audit now hard-fails drift.
  3. **Curation burn-down** — every row has effective axis values but ~711 rows carry an `inferred` label on some axis (roadmap Track A). Re-run the audit's **Annotation sources** line for live counts — that JSON is ground truth, never trust a cached number.
  4. **Honorifics as a systematic register axis** (partially encoded today; roadmap Track D, design-gated).
  5. **Pronunciation scoring** — transcript-match stub; phoneme-level scoring needs an owner decision (not feasible client-side/no-backend). Don't attempt it silently (roadmap Track E).
