...
- **App shape:** vanilla static PWA — **no build step, no framework, no `package.json`**. The core is one large file `app.js` (~13.7k lines) loaded as a plain browser script, plus:
  ...
  - `lib/hangul.js`
  - `words_curated_core.js` — 2,001 curated Words rows (`window.HANAPATH_CURATED_WORDS`) — but never trust this cached number; the audit's `Curated words:` line is ground truth
...
- **⚠️ Read this before trusting any "✅ done" in §8/§11: the self-reported scorecard has been wrong four times now**, caught only by independently re-deriving the underlying data rather than trusting the doc:
  ...
  4. **(2026-07-04 cleanup)** The #54 fix itself had two remaining loopholes: the dupe check grouped rows by korean+POS (so the same word re-added under a different POS label was never compared — 언제 existed as both adverb and pronoun "when", 거나 as both ending and particle "or"), and a pair where both rows carried *distinct* senseKeys was fully exempt even with identical meanings (배 had two "stomach" rows keyed `stomach`/`belly`; 정리's cleanup/organization "split" was one sense wearing two keys). The scorecard was claiming "83 lemmas genuinely split"; the true count was 80. Fixed: the 4 duplicate rows were merged/removed and the audit now compares the whole same-surface group regardless of POS and hard-fails identical core glosses even when senseKeys differ.
...
- **What remains to finalize the Words section** — the authoritative, ordered checklist is **teaching spec §9**. Summary:
  1. **M2 sense split** — continue the high-frequency polyseme list in spec §9. 90 lemmas are genuinely split as of 2026-07-04 (roadmap Tracks B1–B2 done; B3–B5 remain — 15 lemmas: 떨어지다/오르다/나오다/놓다/세우다/끊다/빼다/남다/감다/달다/바람/밝다/세다/피다/두다); the full lemma list lives in spec §8 (single source of truth — don't maintain a copy here).
