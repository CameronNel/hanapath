...
- **App shape:** vanilla static PWA — **no build step, no framework, no `package.json`**. The core is one large file `app.js` (~13.7k lines) loaded as a plain browser script, plus:
  ...
  - `lib/hangul.js`
  - `words_curated_core.js` — 2,020 curated Words rows (`window.HANAPATH_CURATED_WORDS`) — but never trust this cached number; the audit's `Curated words:` line is ground truth
...
- **What remains to finalize the Words section** — the authoritative, ordered checklist is **teaching spec §9**. Summary:
  1. **M2 sense split** — continue the high-frequency polyseme list in spec §9. 99 lemmas are genuinely split as of 2026-07-04 (roadmap Tracks B1–B4 done — B4's 남다 was correctly declined-in-writing as too thin a distinction for beginner level; only B5 remains — 5 lemmas: 바람/밝다/세다/피다/두다); the full lemma list lives in spec §8 (single source of truth — don't maintain a copy here).
  ...
  3. **Curation burn-down** — 575 rows still carry at least one inferred axis; use the Word Bank "Needs curation" filter. Re-run the audit's **Annotation sources** line for live counts — that JSON is ground truth, never trust a cached number.
