...
- **Shipped: Track A (bank foundation, PR #98) + the Sentence Studio (2026-07-06).**
  - Track A: `sentences_core.js` (2,007 rows extracted from the curated Words examples, all `band`/`patternTags` still `inferred`), strict audit `scripts/audit-sentences-data.mjs`, app-shell wiring.
  ...
  - **This covers roadmap B1 (translate core) + C1 (hub) and lays a per-sentence-progress foundation for C3.** The remaining boxes (B2 helper ladder, B3 answer alignment, C2 i+1 gating, C3 real SRS, J1 analytics) each have a labelled `EXTENSION (roadmap <box>)` comment in `app.js` — **extend the studio at those markers, do not rebuild it.** See `docs/SENTENCES_FINAL_ROADMAP.md` §3.5 for the map.
  - Legacy `getSentenceStudyBank()`/`makeSentence*` remain **only** because the Listening tab still shares them (removal is Track I — check Listening first).
