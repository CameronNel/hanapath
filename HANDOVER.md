...
- **What remains to finalize the Words section now is only the owner-gated Track D/E decisions** — the authoritative, ordered checklist is **teaching spec §9**, and the batch-by-batch execution queue for it is [`docs/WORDS_FINAL_ROADMAP.md`](docs/WORDS_FINAL_ROADMAP.md) (one unchecked box = one PR; start there). Summary:
