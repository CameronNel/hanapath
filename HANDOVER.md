...
- **Shipped: Track A (bank foundation, PR #98) + the Sentence Studio + Tracks B/C/D/H/I (2026-07-06 → 07).** Current main tip `a663d49` (PR #121); cache `v244` / `app.js?v=20260707a`. Live per-sentence facts (re-derive before trusting): bank **2,060 rows**, `band` explicit on all 2,060, `patternTags` explicit on 2,007 / inferred on 53 (`s2008–s2060`).
  - Track A: `sentences_core.js` (originally 2,007 rows extracted from the curated Words examples; now 2,060 after the #109 legacy consolidation), strict audit `scripts/audit-sentences-data.mjs`, app-shell wiring.
  - **Sentence Studio** — the Sentences section was rebuilt from scratch as its own subsystem (owner brief: it was "too boiler-heavy and not its own thing"). `renderPracticeView` is now a self-contained hub → 5-question session → summary flow over `HANAPATH_SENTENCES`, with three production-first drills (Translate & Type, Word Builder, Dictation), `state.sentencesProgress` per-sentence records, and a band selector. It no longer routes through the shared quiz engine. Code is one contiguous block in `app.js` under `// --- PRACTICE: SENTENCE STUDIO`; styles are the `.ss-*` rules in `styles.css`.
  - **This covers roadmap B1 (translate core), B2 (helper ladder), and C1 (hub); #108 then landed B3 (LCS token-diff feedback), C2 (i+1 word gating via `focusWordIds`), C3 (per-sentence Leitner SRS reusing `VOCAB_SRS_INTERVALS`), and C4 (Today/stage-menu due-review surfacing).** #121 fixed two review defects in that work: the locked-band "Learn N more words" CTA now targets `data-ss-goto="vocabulary"` (was the invalid `"vocab"`), and the "Reviews due" stat colors via `var(--warn)` (was undefined `var(--warning-color)`). Remaining boxes (J1 analytics, etc.) keep labelled `EXTENSION (roadmap <box>)` comments in `app.js` — **extend at those markers, do not rebuild.** See `docs/SENTENCES_FINAL_ROADMAP.md` §3.5.
  - **#109 (Track H/I) consolidated the legacy mini-banks:** the survival/grammar/verb/honorific/conversation decks still feed the Listening tab via `getSentenceStudyBank()`/`makeSentence*`, but they no longer hard-code English — each now looks its row up in `HANAPATH_SENTENCES` by Korean text (`.find(r => r.korean === …)`) and falls back to `""` on a miss. The #121 review confirmed all 54 such lookups resolve (0 misses / 0 empty); the outstanding item is a **browser** smoke-test of that tab (see the Sentences handover, task 3). Full retirement is still Track I.
- **2026-07-08 foundation rails for Gemini follow-ups:** Sentence Studio now has app-level foundations for pattern lessons, Shadow, Transform, and sentence analytics. Pattern lessons open from the hub and run concept → examples → Translate/Build checks. Shadow mode reuses the accepted browser SpeechRecognition transcript-match stub. Transform mode uses `HANAPATH_INFLECT` and only serves rows with a generated sentence-level target. `recordSentenceResult()` emits sentence review events, and `scripts/audit-sentences-foundation.mjs` validates lesson refs plus transform candidate coverage. Gemini-safe work should stay inside the recipes in `docs/SENTENCES_FINAL_ROADMAP.md` §12.5 and keep that audit green.
