...
- **App shape:** vanilla static PWA — **no build step, no framework, no `package.json`**. The core is one large file `app.js` (~13.7k lines) loaded as a plain browser script, plus:
  ...
  - `lib/hangul.js`
  - `words_curated_core.js` — 1,976 curated Words rows (`window.HANAPATH_CURATED_WORDS`)
...
- **What remains to finalize the Words section** — the authoritative, ordered checklist is **teaching spec §9**. Summary:
  1. **M2 sense split** — continue the high-frequency polyseme list in spec §9. Recent batches shipped: 검사 examination/prosecutor, 감독 director/supervisor, 가정 household/assumption, 문구 stationery/phrase; 과정 process/course, 인상 impression/price increase; 시계 clock/watch, 머리 head/hair, 목 throat/neck, 가슴 heart/chest.
  2. **Curriculum polish** — successive dedupe passes (#51, #53, #54) left **54 lessons with subtitles claiming more words than they contain** and **11 thin lessons (1–2 words)** to fold into same-stage siblings; derive subtitles from actual counts and add an audit check so it can't drift again.
