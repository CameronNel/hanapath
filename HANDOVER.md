...
- **What remains to finalize the Words section** — the authoritative, ordered checklist is **teaching spec §9**. Summary:
  ...
  3. **Curation burn-down** — 621 rows still carry at least one inferred axis; use the Word Bank "Needs curation" filter. Re-run the audit's **Annotation sources** line for live counts — that JSON is ground truth, never trust a cached number.
