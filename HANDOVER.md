...
- **Shipped so far: Track A only (foundation, PR #98, 2026-07-05)** — `sentences_core.js` (2,007 rows extracted from the curated Words examples, all `band`/`patternTags` still `inferred`), the strict audit `scripts/audit-sentences-data.mjs`, and app-shell wiring. **The user-facing engine has not shipped:** the practice tab still runs the legacy `renderPracticeView` level-rail shell over the in-`app.js` mini-banks, and nothing consumes the bank yet. Next box: **B1 Translate & Type** — do not rebuild the bank.
