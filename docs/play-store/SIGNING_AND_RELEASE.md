...
3. Record the certificate fingerprints (safe to write down; they are public):
   ...
   passwords next to the keystore file itself.
...
- Add **Required reviewers** and put yourself in it. Every release run will
  ...
  builds.
- Add the four **environment secrets** (environment secrets, not repository
...
4. The workflow, from a clean checkout of that `main` commit:
   ...
   - restores the keystore from the base64 secret into a `0600` temp file,
     fails fast if the alias/password is wrong, builds `bundleRelease`, and
   ...
   - verifies the AAB signature (`jarsigner -verify -strict`) and records the
...
- Secrets are referenced only in the four steps that need them and are passed
...
- **Keystore leaked** — reset the upload key through the Play Console
  immediately and rotate all four secrets.
