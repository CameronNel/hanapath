...
- [~] **Track E** — Pattern micro-lessons: data file authored (`sentences_lesson_plan.js`), player wiring pending
...
| 2026-07-07 | Band accuracy sweep s0001-s2060 (A1-A11) | local branches | Swept all 2,060 rows across 11 stacked PRs (v253-v263). Adjusted bands where structure clearly contradicted current band (e.g. promoting honorific-si and formal-nida sentences to Band 5). Borderlines kept. |
| 2026-07-07 | Track E Lesson Data (B) | local branch | Authored sentences_lesson_plan.js containing 12 pattern micro-lesson units (4-5 low-band sentences each); loaded in index.html + sw.js, cache bumped to v264. |
| 2026-07-07 | patternTag Coverage Report (C) | local branch | Generated docs/SENTENCES_TAG_COVERAGE.md analyzing tag frequency, under-taught, and over-represented tags. Docs-only PR branched off main. |
