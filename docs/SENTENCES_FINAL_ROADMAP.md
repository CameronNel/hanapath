...
- [x] **Track H** — Authored expansion **complete (queue closed 2026-07-19)**: 2,117 sentences across 21 batches (s2061–s4177) covering grammatical and thematic gaps; any further scenario pack requires owner approval of themes.
- [x] **Track I** — Legacy consolidation **complete (2026-07-19)**: mini-bank migration (#109) + the Listening tab now reads the full 4,177-row bank via `getListeningSentenceBank()`; the legacy `getSentenceStudyBank()` aggregation and `pushSentenceStudyItem()` are deleted (browser-verified: sentence choice, dictation, phrase listening, conversation listening all pass)
...
- [x] **H2** [high/codex] Batches 1–21 completed (s2061–s4177; 2,117 authored rows, re-derived 2026-07-19) covering grammar gaps and situational scenario-packs with offline audio. The owner-unlocked queue is **closed**; any further scenario pack requires fresh owner approval of themes.
...
- [x] **I2** [high] Point `getSentenceStudyBank()` (and the listening deck's sentence sources) at the bank; delete now-dead bank constants and any orphaned deck code; re-verify the Listening tab still works (it shares these banks — check `makeSentenceListenQuestion` and the conversation drills before deleting anything they still use).
  - **Final removal shipped 2026-07-19:** the Listening tab's sentence source is now the full bank — `getListeningSentenceBank()` maps all 4,177 `HANAPATH_SENTENCES` rows (was: 53 legacy rows + keep-in-place drill fragments), so each listening level slice grew from ~10 to ~417 items and the distractor token bank from ~500 to 11,762 tokens. `getSentenceStudyBank()` and `pushSentenceStudyItem()` are deleted. The keep-in-place drill banks (`grammarClozeBank`, `grammarRoleBank`, `survivalCloze`, `conversationRepairBank`, `conversationScenarioBank`) and the mini-bank shims (`survivalPhrases`, `grammarSentenceBank`, `verbSentenceBank`, `conversationLineBank` — all already bank-lookup shims for meanings) stay: they still power the survival/grammar/verb/conversation studios, which remain reachable via onboarding goal routing and `getDefaultStudioForLevel()`, and phrase-/conversation-listen deck types. Browser-verified per the §12.5 Track I rail: sentence choice, dictation, phrase listening, and conversation listening all answered correctly in the live UI with zero console errors.
...
| 2026-07-17 | H2 batch 16 (s3251–s3410) | draft PR | **160 authored rows across four packs**: family & relatives, chores & cleaning, complaints & problems, making plans with friends. Bands 1-3, polite-informal (해요체) by default. Gating warnings strictly verified to 0. Foundation 0/0; 25 new lessons, zero ids removed; cache v400; edge-tts audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 15 (s3211–s3250) | merged | **40 authored rows covering Business & Office Korean** (Bands 3-5) — workplace etiquette, presentations, meetings, reports, and emails. Utilizes polite-formal (하십시오체) and polite-informal (해요체) registers with honorifics. Foundation 0/0; 1 new lesson, zero ids removed; cache v399; edge-tts audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 14 (s3181–s3210) | merged | **30 authored rows expanding formal register and honorifics** (Bands 4-5) — workplace & service interactions, appointments, notices, and respectful conversations. Includes 23 rows practicing both formal-polite and honorific, 7 rows practicing polite-informal honorifics, and 7 rows practicing plain formal-polite. Foundation 0/0; 8 new lessons, zero ids removed; cache v398; edge-tts audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 13 (s3021–s3180) | PR #286 | **160 authored rows across four packs**: requests/favors/permission (could-you, would-you-mind, may-I, lend/help), advice/suggestions/warnings (be-careful, you-should, dont-overdo-it, its-good-to), past experiences/storytelling (-(으)ㄴ 적이 있다, 그때, 어렸을 때, 처음으로), reactions/exclamations/nuance (really?, wow, congrats, what-a-relief, I-see, of-course). Bands 1-5, adds high-frequency interactional Korean. Foundation 0/0; 21 new lessons, zero ids removed; cache v397; offline mimic3 KSS audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 12 (s2862–s3020) | PR #285 | **159 authored rows across four situational packs** — bank crosses 3,000: nature/animals/outdoors (mountains/sea/sky, zoo & farm animals, camping/hiking), describing people/appearance (height/face/hair, personality, looks-younger-than), time/dates/routines (daily schedule, before/after, weekdays/weekends, punctuality), opinions/agreeing (in-my-opinion, agree/oppose, that-makes-sense, honestly-not-sure). Bands 1-4, universal content incl. richer B3-4 discussion language. Foundation 0/0; 23 new lessons, zero ids removed; cache v396; offline mimic3 KSS audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 11 (s2704–s2861) | PR #284 | **158 authored rows across four situational packs**: cooking/kitchen (steps, ingredients, seasoning, appliances, tasting), sports/exercise (playing, winning/losing, training, injuries, health), technology/phone (battery/wifi, apps, downloads, glitches, updates), emotions/relationships (moods, happy/sad/angry, love/friendship, thanks/sorry, congratulations). Bands 1-4, universal practical content. Foundation 0/0; 26 new lessons, zero ids removed; cache v395; offline mimic3 KSS audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 10 (s2546–s2703) | PR #283 | **158 authored rows across four situational packs**: banking/post office (accounts, deposits/withdrawals, transfers, parcels/stamps, fees, queues), work/school (meetings, deadlines, overtime, interviews, classes, exams, homework, grades, graduation), weather/small talk (seasons, forecast, temperature, greetings, how-are-you), hobbies/making friends (interests, invitations, name/age, keeping in touch). Bands 1-4, universal practical content. Foundation 0/0; 21 new lessons, zero ids removed; cache v394; offline mimic3 KSS audio (157 clips). All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 9 (s2389–s2545) | PR #282 | **157 authored rows across four situational packs in one PR** (per owner: larger batches): doctor/pharmacy (symptoms, clinic, prescriptions, rest advice), appointments & phone (calling, scheduling, confirming/cancelling), airport/travel (passport, check-in, delays, luggage, hotel, exchange), housing/home (rooms, rent/deposit/contract, moving, repairs, cleaning). Bands 1-4, universal practical content. Foundation 0/0; 22 new lessons, zero ids removed; cache v393; offline mimic3 KSS audio (156 clips). All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 8 (s2350–s2388) | PR #281 | 39 authored **shopping situational pack** — looking for items, sizes/trying on, colors/preferences, prices/discounts, paying, exchanges/refunds; clothing counters (벌/켤레/개). Universal high-utility content. Foundation 0/0; 4 new lessons, zero ids removed; cache v392; offline mimic3 KSS audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 7 (s2311–s2349) | PR #280 | 39 authored **directions & transport situational pack** — asking the way, taxi (address/fare/stop here), bus & subway (which number, get off, transfer, buy tickets), how far / how long / traffic, arriving & getting lost, train/flight/stops. Universal high-utility content. Foundation 0/0; 3 new lessons, zero ids removed; cache v391; offline mimic3 KSS audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 6 (s2272–s2310) | PR #279 | 39 authored **dining-out situational pack** — practical restaurant/café Korean across the full flow (seating, ordering, asking about the menu, requests during the meal, paying): counters (두 개/한 잔/한 병), 주세요 requests, spiciness/taste questions, 계산/카드/영수증. Universal high-utility content, first authored-depth pack now that pattern×band coverage is complete. Foundation 0/0; 7 new lessons, zero ids removed; cache v390; offline mimic3 KSS audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 5 (s2234–s2271) | PR #278 | 38 authored practical sentences clearing the **last thin band cells** — only-man b3, from-buteo b3–4, until-kkaji b3–4 all now ≥5 — plus mid-band lifts for copula-negative-anieyo, propositive-eyo, neg-mot, comparison-boda, neg-an, future-geoyeyo. **Milestone: the gap report's thin-cell list is now empty** — every pattern-tag × band cell has ≥5 sentences and every tag ≥10 overall. Universal/practical subject matter. Content-tag rows gate by key content word (foundation 0/0). Curriculum regenerated deterministically: 4 new lessons, zero ids removed; cache v389; offline mimic3 KSS audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 4 (s2194–s2233) | PR #277 | 40 authored **practical high-frequency survival sentences** (work, food, transport, appointments, requests, money) filling the last thin band cells (gap report §3 thin cells 12→5) — only-man b3, from-buteo/until-kkaji b3–4, future/want/can b4, but-jiman b5, must-ya-dwaeda + neg-mot practical, plus core survival phrases (say-again, how-much, where-is). **Owner clarification (2026-07-17) recorded**: the "trainee" framing is the fast-immersion drilling *method*, not K-pop subject matter — scenario-pack wording in the batch prompt + plan §5 reframed to practical situational packs. Content-tag rows gate by key content word (foundation 0/0). Curriculum regenerated deterministically: 7 new lessons, zero ids removed; cache v388; offline mimic3 KSS audio. All strict audits + foundation/app-shell + Chromium smoke pass. |
| 2026-07-17 | H2 batch 3 (s2154–s2193) | PR #276 | 40 authored rows lifting the sparsest **band cells** (gap report §3, thin cells 20→12): want-go-sipda / can-su-itda / comparison-boda (the three lowest-total tags) +8 each across bands 2–5, plus propositive-eyo b3 (was 0), also-do b1–2, but-jiman b4–5, neg-an/neg-ji-anta b3. Content-tag rows gate by their key content word (foundation audit 0/0). Curriculum regenerated deterministically: 4 new lessons, zero ids removed; cache v387; offline mimic3 KSS audio. All strict audits + foundation/app-shell + Chromium boot smoke pass. |
| 2026-07-17 | H2 batch 2 (s2114–s2153) | PR #275 | 40 authored rows **filling the three thin grammar units** (plan §5 fills): sn2-grammar-u1 10→24, sn3-grammar-u2 12→26, sn5-grammar-u3 15→27 (each 2–3 → 4 content lessons). u1 particles & copula, u2 negation & connectives, u3 honorifics/questions/formal. Focus words are the units' grammar function-word ids so rows gate to the twin unit with clean coverage (foundation audit 0/0). Curriculum regenerated deterministically: 5 new lessons appended, zero ids removed; cache v386. Audio generated offline (mimic3 KSS). All strict audits (sentences, audio coverage, words, alphabet) + foundation/app-shell + Chromium boot smoke pass. |
| 2026-07-16 | H2 batch 1 (s2062–s2113) | PR #274 | 52 authored rows into the thin pattern-tag cells (copula-negative-anieyo 2→11, propositive-eyo 4→13, from-buteo 5→12, until-kkaji 7→13, but-jiman 6→13, neg-mot 7→13, only-man 4→12, future-geoyeyo 12→17); regenerated gap report now shows **zero tags under the 10-row baseline**. Bands 1–3, full schema, explicit both axes, `source: "authored"`. Curriculum regenerated deterministically: 7 new lessons appended, zero lesson/unit ids removed; cache v385. Audio generated fully offline in-session via `generate_assets.py --backend mimic3` (pinned CC0 KSS voice, SHA-verified; edge-tts was unusable — the sandbox proxy rejects WebSocket upgrades); fixed a latent stdout-flush truncation in `audit-audio-coverage.mjs --manifest-json` found by that run. All strict audits (sentences, audio coverage, words, alphabet) plus foundation/app-shell and the Chromium boot smoke pass. |
| 2026-07-19 | H2 batches 13-21 + Track H closure | PRs #286 + direct merges (batch 14: fd3ad572, 15: 5fdde594, 16: 54afcc43, 17: e3bc5c1c, 18: 63c5e360, 19: 111c70a2, 20: 94da50a7, 21: 35bac9e2) | **Track H queue closed.** Batch 13 (s3021-s3180, 160 rows, four interactional packs); batch 14 (s3181-s3210, formal register & honorific expansion); batch 15 (s3211-s3250, business/office); batch 16 (s3251-s3410, family/chores/complaints/plans); batches 17-21 (s3411-s4177, 767 rows: pets/driving/money/holidays, romance/kids/salon/sports, gardening/music/art/community, recycling/fitness/jobs/languages, hygiene/directions/rooms). Re-derived totals 2026-07-19: bank 4,177 rows (2,007 words-core + 53 legacy-app + 2,117 authored s2061-s4177 across 21 batches); curriculum 75 units / 628 content lessons / 75 checkpoints; 9,704 finite prompts, 0 rows under 2 presentations; audio manifest 34,976 targets, 0 missing/invalid; gap report regenerated at 4,177 rows (no tag <10, no thin target-band cells). All strict audits green on `main`. Any further scenario pack requires fresh owner approval of themes. |
| 2026-07-19 | I2 final (Track I closure) | this PR | **Listening tab repointed at the full bank; legacy aggregation deleted.** `getListeningSentenceBank()` maps all 4,177 `HANAPATH_SENTENCES` rows (korean/english/voiceText/band/tokenCount); `getSentenceStudyBank()` + `pushSentenceStudyItem()` removed. Listening level slices grew ~10 -> ~417 items; distractor token bank -> 11,762 tokens (also feeds Sentence Studio tile pools — verified 3 answers + 5 distractors). Keep-in-place drill banks and the survival/grammar/verb/conversation studios untouched (still reachable via onboarding + level defaults). Cache v409 / app.js?v=20260719c. Verified: node --check; all strict audits + foundation + app-shell + audio-coverage + learning-questions green; live Chromium run — sentence choice, dictation (Hangul tile keyboard + Check), phrase listening, conversation listening each answered correctly (4/4), zero console errors. |
