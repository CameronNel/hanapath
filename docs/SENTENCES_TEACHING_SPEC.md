...
| 12 | Legacy mini-bank migration + dead-code removal | ✅ **done (2026-07-19)** — mini-bank data migrated (#109) and the Listening tab now reads the full 4,177-row bank via `getListeningSentenceBank()`; the legacy `getSentenceStudyBank()` aggregation + `pushSentenceStudyItem()` are deleted. The `makeSentence*` question makers remain by design — they are the live, now fully bank-backed Listening drills. Browser-verified: sentence choice, dictation, phrase listening, conversation listening (roadmap §11 I2 note) |
...
> reports 3,410 explicit rows and 0 inferred rows for both `band` and
> `patternTags`.
