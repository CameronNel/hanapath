...
| 7 | Pattern-tag curation (inferred → explicit) | ✅ **done** — 3,250 rows explicit / 0 inferred; the original accuracy sweep covers s0001–s2060, and s2061-s3250 are explicitly authored and audited. |
...
| 11 | Authored expansion batches (gap-driven) | ✅ **done** — 1,190 sentences added across 15 authored batches (s2061–s3250) covering grammatical and thematic gaps; any further scenario pack requires owner approval. |
...
> manifest contains 36,940 targets and the strict audit reports 36,940 valid
> local assets, 0 missing mappings, and 0 invalid assets. The finite-question
> audit covers all 3,250 sentence rows at least twice (7,850 presentations), all
...
> reports 3,250 explicit rows and 0 inferred rows for both `band` and
> `patternTags`.
