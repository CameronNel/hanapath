...
| **Sense-level polysemy** | 🟡 | **90 lemmas** carry genuine multi-sense rows with distinct `senseKey`/`senseNo`. Verbs/adjectives: 쓰다 write/wear/use, 하다 do/say, 보다 see/try, 나다 occur/sprout, 타다 ride/burn, 내리다 get-off/fall, 일어나다 wake/stand, 맞다 correct/get-hit, 뛰다 run/jump, 가다 go/time-passing, 싸다 cheap/pack, 들다 hold/cost, 걸리다 take-time/catch-illness, 지다 lose/sunset, 걸다 hang/call, 서다 stand/stop, 치다 hit/play-instrument, 풀다 untie/solve, 돌다 turn/go-around, 재다 measure/be-calculating, 짜다 salty/make-a-plan, 차다 kick/be-full, 빠지다 fall-into/be-omitted, 붙다 stick/pass-an-exam, 있다 have/exist, 없다 not-have/not-exist, 보내다 send/spend-time, 잡다 catch-grab/set-arrange, 크다 big(adj)/grow-up(verb), 쉬다 rest/breathe, 열다 open/hold-event, 짓다 build/make-prepare, 살다 live-reside/be-alive. Nouns/counters/other: 시장 market/mayor/hunger, 배 stomach/pear/boat, 말 speech/horse, 거리 distance/street, 불 light/fire, 길 road/way-method, 초 second-time/candle, 것 independent/bound noun, modifier `(으)ㄴ`, 눈 snow/eye, 다리 bridge/leg, 밤 night/chestnut, 차 tea/car, 사과 apple/apology, 밥 meal/rice, 시간 time/hour, 아침 morning/breakfast, 점심 lunch/noon, 저녁 evening/dinner, 병 bottle/illness/counter, 일기 diary/weather, 양식 Western-food/form, 검사 examination/prosecutor, 감독 director/supervisor, 가정 household/assumption, 문구 stationery/phrase, 과정 process/course, 인상 impression/price-increase, 시계 clock/watch, 머리 head/hair, 목 throat/neck, 가슴 heart/chest, 분 minute/honorific-counter, 새 bird/new, 통화 phone-call/currency, 사고 accident/thinking, 연기 acting/smoke, 전기 electricity/biography, 이사 moving/director, 소식 news/eating-light, 돌 stone/first-birthday, 일식 Japanese-food/solar-eclipse, 중식 Chinese-food/lunch, 구조 structure/rescue, 조직 organization/tissue, 모델 fashion-model/prototype, 코드 programming-code/dress-code, 대기 atmosphere/waiting, 시점 time-point/viewpoint, 차례 turn/ancestral-rites/table-of-contents. **Correction (PR #54, 2026-07-03):** the previously reported "128 rows / 94 lemmas" figure was wrong — 74 of those rows were accidental duplicate content disguised as polysemy (a second copy of the same word/meaning tagged with a fabricated `senseKey` like `"small"`/`"busy"`/`"hobby"` purely to dodge the duplicate-content audit, which fully exempted a whole same-surface group the moment *any* row in it had a `senseKey`). **Correction 2 (2026-07-04 cleanup):** the batches above briefly reported "83 lemmas", but 3 of those were duplicates that had survived two remaining audit loopholes — 배 had a second "stomach" row hiding behind a different senseKey (`belly`), and 언제/거나 each existed twice under different POS labels (the dupe check grouped by korean+POS, so cross-POS copies were never compared); 정리's cleanup/organization "split" was also one sense wearing two keys and was merged. The audit now compares the whole same-surface group regardless of POS and hard-fails identical core glosses even when senseKeys differ. Roadmap Tracks B1 (거리/불/잡다/길/크다) and B2 (쉬다/열다/짓다/살다/초) are now done — see §9. Most polysemous lemmas remain untagged; real M2 authoring continues per §9 |
...
| Vocabulary volume | ✅ | 2,001 unique curated senses (1,918 post-#54, +77 across fifteen M2 batches, −4 in the 2026-07-04 dedupe cleanup, +5 in roadmap Track B1, +5 in Track B2), still comfortably inside the Core 1000 / Core 2000 target band, with every curated row assigned to a lesson |
...
1. **M2 sense split (the one open milestone — real authoring, not tagging).**
   **90 lemmas** have genuine multi-sense rows — the full current list lives in
   §8; that table is the single source of truth for which lemmas are split
   (don't maintain a second list here). Roadmap Track B1 (거리/불/잡다/길/크다
   un-bundled) and Track B2 (쉬다/열다/짓다/살다/초 given authored second
   senses) shipped 2026-07-04; Tracks B3–B5 remain. Batches so far: 2026-07-03
   (눈/다리/밤/차/맞다, slotted into thin lessons to help §9 item 2 too; also
   fixed a bug where 부르다's verb row wrongly bundled the adjective sense's
   gloss into its own meaning), then fifteen 2026-07-04 batches covering the
   verbs 사과/뛰다/가다/싸다/들다/걸리다/지다/걸다/서다/치다/풀다/돌다/재다/
   짜다/차다/빠지다/붙다/있다/없다/보내다 and the nouns 사고/연기/전기/이사/
   소식/밥/시간/아침/점심/저녁/병/일기/양식/검사/감독/가정/문구/과정/인상/
   시계/머리/목/가슴/분/새/통화/돌/일식/중식/구조/조직/모델/코드/대기/시점/
   차례, plus the missing 배 pear sense (풀다's "solve" sense was slotted into
   `w218-theme-264`, resolving the one thin lesson #59 had left unfoldable).
   A 2026-07-04 cleanup pass then removed 4 rows those batches had left or
   revealed as duplicates (배 second-stomach, 언제 and 거나 cross-POS copies,
   정리 one-sense "split") and closed the audit loopholes that had admitted
   them — see §8's Correction 2.
   ...
   *unrelated* word 알다 "to know," which only look similar to a learner, not
   ...
   audit now hard-fails identical senseKeys and identical core glosses across
   the whole same-surface group (regardless of POS), but it cannot judge
   semantics — read the
...
| **M5** | **Authoring to Core 1000** — grow ~230 → 800–1,000 senses vs official level-1 list | M1 (schema); M3 (leverage) | `words_curated_core.js`, `words_lesson_plan.js` | ≥800 senses; numbers/counters + Sino-Korean families as explicit themes | L (ongoing) | ✅ done — 2,001 unique curated senses (corrected to 1,918 post-#54 after 74 disguised duplicates were removed, see §8; +77 across fifteen M2 batches, −4 in the 2026-07-04 dedupe cleanup, +5 in Track B1, +5 in Track B2); strict audit clean; no orphan words |
