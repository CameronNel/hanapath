...
| Vocabulary volume | ✅ | 1,995 unique curated senses (1,918 post-#54, +77 across fifteen M2 batches), still comfortably inside the Core 1000 / Core 2000 target band, with every curated row assigned to a lesson |
...
1. **M2 sense split (the one open milestone — real authoring, not tagging).**
   83 lemmas have genuine multi-sense rows (§8). Fifteen batches so far:
   ...
   전기 electricity/biography, 이사 moving/director, 소식 news/eating-light); 2026-07-04 batch 5 (과정 process/course, 인상 impression/price increase); 2026-07-04 batch 9 (구조 structure/rescue, 조직 organization/tissue, 모델 fashion/prototype, 코드 programming/dress code, slotted into body-health, daily-objects-tech, and clothing lessons); 2026-07-04 batch 10 (대기 atmosphere/waiting, 시점 point in time/viewpoint, 차례 turn/ancestral rites/table of contents).
