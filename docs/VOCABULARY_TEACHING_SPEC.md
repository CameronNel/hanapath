...
| **Sense-level polysemy** | 🟡 | 24 lemmas carry genuine multi-sense rows with distinct `senseKey`/`senseNo` (쓰다 write/wear/use, 시장 market/mayor/hunger, 배 stomach/pear/boat, 말 speech/horse, 타다 ride/burn, 내리다 get-off/fall, 일어나다 wake/stand, 것 independent/bound noun, modifier `(으)ㄴ`, 하다 do/say, 보다 see/try, 나다 occur/sprout, 눈 snow/eye, 다리 bridge/leg, 밤 night/chestnut, 차 tea/car, 맞다 correct/get-hit, plus 사과 apple/apology, 뛰다 run/jump, 가다 go/time-passing, 싸다 cheap/pack, and three brand-new multi-sense verbs authored from scratch — 들다 hold/cost, 걸리다 take-time/catch-illness, 지다 lose/sunset — added 2026-07-04). **Correction (PR #54, 2026-07-03):** the previously reported "128 rows / 94 lemmas" figure was wrong — 74 of those rows were accidental duplicate content disguised as polysemy (a second copy of the same word/meaning tagged with a fabricated `senseKey` like `"small"`/`"busy"`/`"hobby"` purely to dodge the duplicate-content audit, which fully exempted a whole same-surface group the moment *any* row in it had a `senseKey`). Removed; the audit's exemption logic is now closed (a shared/identical `senseKey` across rows is a hard error) so this can't silently recur. Most polysemous lemmas remain untagged; real M2 authoring continues per §9 |
...
| Vocabulary volume | ✅ | 1,933 unique curated senses (1,918 post-#54, +5 from the 2026-07-03 M2 batch, +10 from the 2026-07-04 M2 batch), still comfortably inside the Core 1000 / Core 2000 target band, with every curated row assigned to a lesson |
...
1. **M2 sense split (the one open milestone — real authoring, not tagging).**
   24 lemmas have genuine multi-sense rows (§8). Two batches so far:
   ...
   adjective sense's gloss into its own meaning) and 2026-07-04 (사과/뛰다/
   가다/싸다 split from single bundled rows, plus 들다/걸리다/지다 authored as
   brand-new multi-sense verbs since they weren't in the curated set at all).
   Still-open candidates for more batches: 걸다 (hang/call), 안다 (embrace —
   check it isn't confused with 알다 "to know"), 물다 (bite/pay a fee), 서다
   (stand/stop), 치다 (hit/play an instrument), 재다 (measure/boast), 짜다
   (salty/to weave-plan), 차다 (kick/be cold/be full), 풀다 (untie/solve),
   빠지다 (fall into/be omitted), 붙다 (stick/pass an exam), 돌다 (turn/go
   crazy) — none are in the curated set yet, so each is new-word authoring
   like 들다/걸리다/지다, not a split. For every case: give each real sense
   its own row, example, and review identity. ⚠️ A `senseKey` must mark a
   **genuinely distinct meaning**; the audit now hard-fails identical
...
Status is honest as of 2026-07-03 (post-#54).
...
| **M2** | **Sense split** — per-sense rows for polysemous lexemes (보다, 하다, 나다…) | M1 | `words_curated_core.js`, `app.js` (Word Bank + lesson render) | high-freq polysemes split; sense shown in bank + lessons | M | 🟡 partial — 24 lemmas genuinely split (see §8; 눈/다리/밤/차/맞다 added 2026-07-03, 사과/뛰다/가다/싸다/들다/걸리다/지다 added 2026-07-04). The "128 rows / 94 lemmas" figure previously reported here was wrong (74 disguised-duplicate rows, corrected in #54) and has been removed. Most polysemous lemmas remain untagged; real work continues per §9 item 1 |
...
| **M5** | **Authoring to Core 1000** — grow ~230 → 800–1,000 senses vs official level-1 list | M1 (schema); M3 (leverage) | `words_curated_core.js`, `words_lesson_plan.js` | ≥800 senses; numbers/counters + Sino-Korean families as explicit themes | L (ongoing) | ✅ done — 1,933 unique curated senses (corrected to 1,918 post-#54 after 74 disguised duplicates were removed, see §8; +15 from the two 2026-07-03/04 M2 batches); strict audit clean; no orphan words |
