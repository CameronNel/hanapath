...
| Vocabulary volume | ✅ | 1,980 unique curated senses (1,918 post-#54, +62 across eleven M2 batches), still comfortably inside the Core 1000 / Core 2000 target band, with every curated row assigned to a lesson |
...
1. **M2 sense split (the one open milestone — real authoring, not tagging).**
   69 lemmas have genuine multi-sense rows (§8). Eleven batches so far:
   ...
   전기 electricity/biography, 이사 moving/director, 소식 news/eating-light); 2026-07-04 batch 5 (과정 process/course, 인상 impression/price increase).
   ...
   *unrelated* word 알다 "to know," which only look similar to a learner, not
   ...
   previous batch 3: 과정 process/course, 인상 impression/price increase; current batch: 분 minute/honorific counter, 새 bird/new, 통화 phone call/currency; plus one extra sense in the already-split 배 belly/pear/boat trio. ⚠️ A `senseKey` must mark a **genuinely distinct meaning**; the
...
| **M2** | **Sense split** — per-sense rows for polysemous lexemes (보다, 하다, 나다…) | M1 | `words_curated_core.js`, `app.js` (Word Bank + lesson render) | high-freq polysemes split; sense shown in bank + lessons | M | 🟡 partial — 69 lemmas genuinely split (see §8; 눈/다리/밤/차/맞다 added 2026-07-03, twenty-two more — 사과/뛰다/가다/싸다/들다/걸리다/지다/걸다/서다/치다/풀다/돌다/재다/짜다/차다/빠지다/붙다/사고/연기/전기/이사/소식 — added 2026-07-04; plus five early noun splits in the previous batch — 밥/시간/아침/점심/저녁; plus four more in the previous batch — 검사/감독/가정/문구; plus two more in the previous batch 3 — 과정/인상; plus four more in the previous batch — 시계/머리/목/가슴; plus three new split lemmas in the current batch — 분/새/통화; plus one extra sense in the already-split 배 trio). The "128 rows / 94 lemmas" figure previously reported here was wrong (74 disguised-duplicate rows, corrected in #54) and has been removed. Most polysemous lemmas remain untagged; real work continues per §9 item 1 |
...
| **M5** | **Authoring to Core 1000** — grow ~230 → 800–1,000 senses vs official level-1 list | M1 (schema); M3 (leverage) | `words_curated_core.js`, `words_lesson_plan.js` | ≥800 senses; numbers/counters + Sino-Korean families as explicit themes | L (ongoing) | ✅ done — 1,980 unique curated senses (corrected to 1,918 post-#54 after 74 disguised duplicates were removed, see §8; +62 across eleven M2 batches); strict audit clean; no orphan words |
