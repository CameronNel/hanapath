...
| Vocabulary volume | ✅ | 1,991 unique curated senses (1,918 post-#54, +73 across fourteen M2 batches), still comfortably inside the Core 1000 / Core 2000 target band, with every curated row assigned to a lesson |
...
1. **M2 sense split (the one open milestone — real authoring, not tagging).**
   80 lemmas have genuine multi-sense rows (§8). Fourteen batches so far:
...
| **M5** | **Authoring to Core 1000** — grow ~230 → 800–1,000 senses vs official level-1 list | M1 (schema); M3 (leverage) | `words_curated_core.js`, `words_lesson_plan.js` | ≥800 senses; numbers/counters + Sino-Korean families as explicit themes | L (ongoing) | ✅ done — 1,991 unique curated senses (corrected to 1,918 post-#54 after 74 disguised duplicates were removed, see §8; +73 across fourteen M2 batches); strict audit clean; no orphan words |
