...
| **Declared checkpoints all generatable** | 🟡 | Was 24 dead lesson-checkpoint combinations when found (2026-07-05). **Track F2 shipped the same day**: `makeWordSentenceBlank` now consults the inflection engine (generated forms + -아/어 infinitive + stem connectives, with shape-matched conjugated distractors), reviving all 18 dead `sentence-blank` checkpoints — unblankable rows fell 280 → 44. **6 structurally dead checkpoints remain** (form drills in w17-tense-negation-01/w18-noun-modification-01, function-usage in w19-honorifics-01/w19-irregular-families-01), queued for removal in Track F3, with audit enforcement in F4 |
