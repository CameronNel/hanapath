...
2. **Curriculum polish after the dedupe passes — ✅ done (2026-07-03).** All 51
   ...
   again.
3. **Curation queue burn-down.** Every row has effective values on every axis, but 531 rows still carry at least one `inferred` axis (`register` 523,
   `speechLevel` 525, `morphTag` 521, `originType` 191; 513 rows are inferred
   on `register`+`speechLevel`+`morphTag` simultaneously). Use the Word Bank's
   **Needs curation** filter + **Curation priority** sort to verify in batches;
   re-run the audit and read its **Annotation sources** line for current counts.
...
| **M5** | **Authoring to Core 1000** ? grow ~230 ? 800?1,000 senses vs official level-1 list | M1 (schema); M3 (leverage) | `words_curated_core.js`, `words_lesson_plan.js` | ?800 senses; numbers/counters + Sino-Korean families as explicit themes | L (ongoing) | ? done ? 2,015 unique curated senses (corrected to 1,918 post-#54 after 74 disguised duplicates were removed, see ?8; +97 across seventeen M2 batches); strict audit clean; no orphan words |
