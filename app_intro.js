(() => {
  ...
  // Word-synced lyric timeline generated by scripts/intro/generate_intro_narration.py.
  const TIMELINE_SCENES = Array.isArray(externalConfig.scenes)
    && externalConfig.scenes.every((scene) => Array.isArray(scene.lines))
    ? externalConfig.scenes
    : null;
  const CHIP_AUDIO = externalConfig.chips && typeof externalConfig.chips === "object"
    ? externalConfig.chips
    : {};
  // Spoken-word fallback script (browser speechSynthesis) used when the
  // studio voiceover cannot load. The words match the lyric timeline.
  const DEFAULT_SCENES = [
    {
      ...
      utterances: [
        { lang: "ko-KR", text: "안녕하세요!" },
        { lang: "en-US", text: "Korean can feel like a wall of unfamiliar sounds and symbols.", pauseBefore: 240 },
      ],
    },
    {
      id: "hangul",
  ...
  const speechTotalDuration = scenes.reduce((sum, scene) => sum + Number(scene.duration || 0), 0);
  const timelineDuration = Number(externalConfig.duration || 0);
  const speechSupported = "speechSynthesis" in window && "SpeechSynthesisUtterance" in window;
  const reducedMotion = window.matchMedia
    && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  // Flat list of lyric lines with hold windows: a line stays on screen until
  // the next line begins (or its scene ends), so gaps never blank the stage.
  const lyricLines = [];
  if (TIMELINE_SCENES) {
    TIMELINE_SCENES.forEach((scene, sceneIndex) => {
      scene.lines.forEach((line) => {
        lyricLines.push({ ...line, sceneIndex });
      });
    });
    lyricLines.forEach((line, index) => {
      const next = lyricLines[index + 1];
      line.holdUntil = next ? next.start : Number.POSITIVE_INFINITY;
    });
  }
  const runtime = {
    root: null,
  ...
  function icon(name) {
    const icons = {
      ...
      spark: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2l1.9 6.1L20 10l-6.1 1.9L12 18l-1.9-6.1L4 10l6.1-1.9z"/></svg>',
    };
    return icons[name] || "";
  }
  function chip(text, extraClass = "") {
    const safe = escapeHtml(text);
    return `<button type="button" class="app-intro-chip ${extraClass}" data-intro-say="${safe}" lang="ko" aria-label="Play ${safe}">${safe}</button>`;
  }
  function sceneMarkup() {
    return `
      ...
      <section class="app-intro-scene app-intro-scene--words" data-intro-scene="words" aria-label="Korean vocabulary becoming familiar">
        <div class="app-intro-word-orbit">
          ${chip("사람")}${chip("물")}${chip("오늘")}${chip("먹다")}${chip("가다")}
        </div>
        <div class="app-intro-morph-card">
          ...
          <div class="app-intro-morph-forms" aria-label="The Korean verb gada changing form">
            ${chip("가다")}${chip("가요")}${chip("갔어요")}
          </div>
          <p>Make them yours. <small>Tap any word to hear it.</small></p>
        </div>
      </section>
      ...
      <section class="app-intro-scene app-intro-scene--possible" data-intro-scene="possible" aria-label="Korean changing from a barrier into a clear path">
        <div class="app-intro-ordered-korean" lang="ko">
          ${chip("한")}${chip("국")}${chip("어")}${chip("를")}${chip("배")}${chip("워")}${chip("요")}
        </div>
  ...
  function buildRoot() {
    ...
    const startHint = MASTER_AUDIO_SRC
      ? "Featuring a natural Korean voice · 한국어 음성"
      : (speechSupported ? "Uses your browser voice for this preview" : "Captions included");
    root.innerHTML = `
      <div class="app-intro-shell">
        <div class="app-intro-ambient" aria-hidden="true">
          <span></span><span></span><span></span>
        </div>
        <div class="app-intro-grain" aria-hidden="true"></div>
        <div class="app-intro-letterbox app-intro-letterbox--top" aria-hidden="true"></div>
        <div class="app-intro-letterbox app-intro-letterbox--bottom" aria-hidden="true"></div>
        ...
        <main class="app-intro-stage" data-intro-stage>
          ${sceneMarkup()}
        ...
        <div class="app-intro-caption" aria-live="polite" aria-atomic="true">
          <div class="app-intro-lyrics" data-intro-caption></div>
        </div>
        <footer class="app-intro-controls">
          ...
          <div class="app-intro-progress" role="progressbar" aria-label="Introduction progress" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0" data-intro-seek>
            <span data-intro-progress></span>
            <span class="app-intro-progress-ticks" data-intro-ticks aria-hidden="true"></span>
          </div>
  ...
  function buildSceneTicks(root) {
    const holder = root.querySelector("[data-intro-ticks]");
    if (!holder || !TIMELINE_SCENES || !timelineDuration) return;
    holder.innerHTML = TIMELINE_SCENES.slice(1)
      .map((scene) => `<i style="left:${((scene.start / timelineDuration) * 100).toFixed(2)}%"></i>`)
      .join("");
  }
  function bindEvents(root) {
    ...
    root.querySelector("[data-intro-stage]")?.addEventListener("click", (event) => {
      const chipButton = event.target.closest("[data-intro-say]");
      if (chipButton) playChip(chipButton);
    });
    root.querySelector("[data-intro-seek]")?.addEventListener("pointerdown", onSeek);
    if (!reducedMotion) {
      const shell = root.querySelector(".app-intro-shell");
      shell?.addEventListener("pointermove", onParallax);
      shell?.addEventListener("pointerleave", () => setParallax(0, 0));
    }
  ...
  function onParallax(event) {
    ...
    cancelAnimationFrame(runtime.parallaxFrame);
    runtime.parallaxFrame = requestAnimationFrame(() => setParallax(x, y));
  }
  function setParallax(x, y) {
    const shell = runtime.root?.querySelector(".app-intro-shell");
    if (!shell) return;
    shell.style.setProperty("--intro-px", x.toFixed(3));
    shell.style.setProperty("--intro-py", y.toFixed(3));
  }
  function onSeek(event) {
    if (runtime.mode !== "audio" || !runtime.audio || !runtime.playing) return;
    const bar = event.currentTarget;
    const rect = bar.getBoundingClientRect();
    const seekTo = (clientX) => {
      const ratio = Math.min(1, Math.max(0, (clientX - rect.left) / rect.width));
      const duration = runtime.audio?.duration;
      if (!duration) return;
      runtime.audio.currentTime = ratio * duration;
      if (runtime.paused) {
        // Keep the frozen frame honest while scrubbing.
        const nowMs = runtime.audio.currentTime * 1000;
        const targetScene = sceneIndexForTime(nowMs);
        if (targetScene !== runtime.currentScene) setScene(targetScene);
        updateLyrics(nowMs);
        setProgress((nowMs / (duration * 1000)) * 100);
      }
    };
  ...
  function playChip(button) {
    const text = button.getAttribute("data-intro-say") || "";
    if (!text) return;
    button.classList.remove("is-saying");
    // Force a restart of the pulse animation on rapid re-taps.
    void button.offsetWidth;
    button.classList.add("is-saying");
    const entry = CHIP_AUDIO[text];
    const master = runtime.audio;
    const duck = (on) => {
      if (!master) return;
      clearTimeout(runtime.chipRestore);
      master.volume = on ? 0.22 : 1;
    };
    if (entry && entry.src) {
      try {
        if (runtime.chipAudio) runtime.chipAudio.pause();
        const clip = new Audio(entry.src);
        runtime.chipAudio = clip;
        duck(true);
        clip.addEventListener("ended", () => {
          button.classList.remove("is-saying");
          runtime.chipRestore = window.setTimeout(() => duck(false), 140);
        }, { once: true });
        clip.play().catch(() => {
          duck(false);
          speakChipFallback(text, button);
        });
        return;
      } catch (_error) {
        duck(false);
  ...
  function speakChipFallback(text, button) {
    if (!speechSupported) {
      button.classList.remove("is-saying");
      return;
    }
    ...
    utterance.rate = 0.82;
    const voice = chooseVoice("ko-KR");
    if (voice) utterance.voice = voice;
    utterance.onend = () => button.classList.remove("is-saying");
    utterance.onerror = () => button.classList.remove("is-saying");
  ...
  function openIntro(options = {}) {
    ...
    root.classList.remove("is-playing", "is-paused", "is-complete", "captions-off");
    ...
    if (MASTER_AUDIO_SRC && !runtime.audio) {
      // Warm the voiceover so pressing Play starts instantly.
      try {
        const audio = new Audio(MASTER_AUDIO_SRC);
        audio.preload = "auto";
        audio.load();
      } catch (_error) { /* preloading is best-effort */ }
    }
  ...
  function resetPlayback() {
    ...
    runtime.lyricLineIndex = -1;
    runtime.lyricWordIndex = -1;
    if (runtime.root) {
  ...
  function startPlayback() {
    if (!runtime.root) return;
    runtime.root.classList.remove("is-awaiting-start", "is-complete");
  ...
  function stopPlayback() {
    ...
    if (runtime.chipAudio) {
      runtime.chipAudio.pause();
      runtime.chipAudio = null;
    }
    if (runtime.audio) {
      runtime.audio.pause();
  ...
  function tickProgress() {
    ...
    const loop = () => {
      ...
      setProgress(((prior + planned) / speechTotalDuration) * 100);
  ...
  function setCaption(value) {
    ...
    caption.classList.toggle("is-plain", Boolean(value));
  }
  function renderLyricLine(index) {
    const caption = runtime.root?.querySelector("[data-intro-caption]");
    if (!caption) return;
    runtime.lyricLineIndex = index;
    runtime.lyricWordIndex = -1;
    ...
    const words = line.words
      .map((word) => `<span class="app-intro-lyric-word">${escapeHtml(word.w)}</span>`)
      .join(" ");
    caption.classList.remove("is-plain");
    caption.innerHTML = `<p class="app-intro-lyric-line${line.lang === "ko" ? " app-intro-lyric-line--ko" : ""}"${line.lang === "ko" ? ' lang="ko"' : ""}>${words}</p>`;
    caption.querySelector(".app-intro-lyric-line")?.style.setProperty("--word-count", String(line.words.length));
  }
  function updateLyrics(nowMs) {
    ...
    for (let index = 0; index < lyricLines.length; index += 1) {
      const line = lyricLines[index];
      if (nowMs >= line.start && nowMs < line.holdUntil) {
        lineIndex = index;
        break;
      }
      if (line.start > nowMs) break;
    }
    if (lineIndex !== runtime.lyricLineIndex) {
      renderLyricLine(lineIndex);
    }
    if (lineIndex === -1) return;
    const line = lyricLines[lineIndex];
    let wordIndex = -1;
    for (let index = 0; index < line.words.length; index += 1) {
      if (nowMs >= line.words[index].t) wordIndex = index;
      else break;
    }
    if (wordIndex === runtime.lyricWordIndex) return;
    runtime.lyricWordIndex = wordIndex;
    const spans = runtime.root?.querySelectorAll(".app-intro-lyric-word") || [];
    spans.forEach((span, index) => {
      span.classList.toggle("is-active", index === wordIndex);
      span.classList.toggle("is-sung", index < wordIndex);
    });
  }
  function sceneIndexForTime(nowMs) {
    if (!TIMELINE_SCENES) return runtime.currentScene;
    let target = 0;
    for (let index = 0; index < TIMELINE_SCENES.length; index += 1) {
      if (nowMs >= Number(TIMELINE_SCENES[index].start || 0)) target = index;
      else break;
    }
    return target;
  }
  ...
  function tickMasterAudio() {
    ...
    const loop = () => {
      ...
      const durationMs = (runtime.audio.duration || (timelineDuration || speechTotalDuration) / 1000) * 1000;
      const nowMs = runtime.audio.currentTime * 1000;
      const targetScene = sceneIndexForTime(nowMs);
      if (targetScene !== runtime.currentScene || !runtime.root?.getAttribute("data-current-scene")) {
        setScene(targetScene);
      }
      updateLyrics(nowMs);
      setProgress((nowMs / Math.max(durationMs, 100)) * 100);
      runtime.frame = requestAnimationFrame(loop);
    };
    runtime.frame = requestAnimationFrame(loop);
  }
