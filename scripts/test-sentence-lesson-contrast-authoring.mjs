...
const selected = selectContrastRow(target, [target, closeContrast, distantContrast]);
check("selects a different live row", selected.id !== target.id);
check("prefers a nearby grammatical contrast", selected.id === "s2");

const entry = buildLessonContrastEntry({ candidate, target, contrast: selected, lesson, sectionOrder: 1 });
...
check("puts target and contrast first", entry.lessonMutation.sentenceIds[0] === "s1" && entry.lessonMutation.sentenceIds[1] === "s2");
check("preserves every original lesson row", lesson.sentenceIds.every((id) => entry.lessonMutation.sentenceIds.includes(id)));
...
check("adds target and contrast study notes", Boolean(entry.studyNotes.s1 && entry.studyNotes.s2));
